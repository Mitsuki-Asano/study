...
## 目的
本プロジェクトは、iPhoneで撮影した植物画像を入力として、  
**多肉植物（succulent） / それ以外の植物（other）** の2クラスに自動分類する  
深層学習（Deep Learning）モデルを構築することを目的とする。
...
学習時に自動的にリサイズ・クロップされ、モデル入力サイズ（224×224）に変換される
...
事前学習済み MobileNetV3 を用いた転移学習を実施
